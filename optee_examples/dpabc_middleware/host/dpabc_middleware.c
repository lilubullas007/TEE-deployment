...
static char ** deflat_list(list_entry * data, uint32_t data_sz, int * num_tags) {
	...
	data_ix = (char*)data;
	data_end = data_ix + data_sz;
	
	char ** ret_val = malloc(1);

	size_t total_sz = 0;
	int i = 0;
	while (data_ix < data_end) {

		current_entry = (list_entry*)data_ix;
		// printf("Data length: %d\n", current_entry->entry_length);
		total_sz += current_entry->entry_length;
		ret_val = realloc(ret_val, total_sz);
		char * temp_buff = malloc(current_entry->entry_length + 1);
		memcpy(temp_buff, &(current_entry->data), current_entry->entry_length);
		temp_buff[current_entry->entry_length] = '\0';
		ret_val[i++] = temp_buff;
		data_ix += current_entry->entry_length + sizeof(list_entry);
		// printf("%d fst: %.*s\n", i++, current_entry->entry_length, temp_buff);

		current_entry = (list_entry*)data_ix;
		// printf("Data length: %d\n", current_entry->entry_length);
		// memcpy(temp_buff, &(current_entry->data), current_entry->entry_length);
		data_ix += current_entry->entry_length + sizeof(list_entry) + sizeof(uint32_t);
		// printf("%d scd: %.*s\n", i++, current_entry->entry_length, temp_buff);
...
DPABC_status DPABC_testADP(DPABC_session * session) {
	...
	printf("Invoking TA to test ADP\n");
	res = TEEC_InvokeCommand(&(session->sess), TA_DPABC_TEST_ADP, &op,
	&err_origin);

	if (res != TEEC_SUCCESS) {
		printf("Command TA_DPABC_TEST_ADP failed: 0x%x / %u\n", res, err_origin);
		return STATUS_WRITE_ERROR;
	}

	printf("TA ADP success\n");

	return STATUS_OK;

}

DPABC_status DPABC_ListStored(DPABC_session * session, char *** tags, int * num_tags) {
        ...
        op.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_OUTPUT,
					 TEEC_NONE,
					 TEEC_NONE, TEEC_NONE);
        uint32_t memsize = DEFAULT_BUFFER_SIZE;
        list_entry* temp = (list_entry*)malloc(memsize);

        while(1)
        {
                op.params[0].tmpref.buffer = temp;
                op.params[0].tmpref.size = memsize;

		printf("Invoking TA to list storede elements\n");

                res = TEEC_InvokeCommand(&session->sess, TA_DPABC_LIST_STORED, &op, &origin);

                uint32_t size = op.params[0].tmpref.size; // actual size for buffer

                if(res == TEEC_SUCCESS)
                {
                        temp = (list_entry*)realloc((void*)temp, size); // no wasted memory
                        // *data = temp;
                        // *data_sz = size;

			*tags = deflat_list(temp, size, num_tags);

			printf("TA to list storede elements success\n");
                ...
                if (res != TEEC_ERROR_SHORT_BUFFER) {
			printf("Command TA_DPABC_LIST_STORED failed: 0x%x / %u\n", res, origin);
                        free(temp);
                        return STATUS_GENERIC_ERROR;     // Something is broken in the TEE
                } // else: TEEC_ERROR_SHORT_BUFFER → retry with larger buffer 

                temp = (list_entry*)realloc((void*)temp, size);
                memsize = size;

        }

        return STATUS_KEY_READ_ERROR; // Ran out of retries
