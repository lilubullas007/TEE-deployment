...
#elif ALLOC == OPTEE
#define shpe_new(A)															\
	A = (shpe_t)TEE_Malloc(1 * sizeof(shpe_st), 0);							\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	bn_new((A)->a);															\
...
#elif ALLOC == OPTEE
#define rsa_new(A)															\
	A = (rsa_t)TEE_Malloc(1 * sizeof(_rsa_st), 0);							\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	bn_null((A)->d);														\
...
#elif ALLOC == OPTEE
#define bdpe_new(A)															\
	A = (bdpe_t)TEE_Malloc(1 * sizeof(bdpe_st), 0);							\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	bn_new((A)->n);															\
...
#elif ALLOC == OPTEE
#define sokaka_new(A)														\
	A = (sokaka_t)TEE_Malloc(1 * sizeof(sokaka_st), 0);						\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	g1_new((A)->s1);														\
...
#elif ALLOC == OPTEE
#define bgn_new(A)															\
	A = (bgn_t)TEE_Malloc(1 * sizeof(bgn_st), 0);							\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	bn_new((A)->x);															\
...
#elif ALLOC == OPTEE
#define ers_new(A)															\
	A = (ers_t)TEE_Malloc(1 * sizeof(ers_st), 0);							\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	ec_new((A)->h);															\
...
#elif ALLOC == OPTEE
#define smlers_new(A)														\
	A = (smlers_t)TEE_Malloc(1 * sizeof(ers_st), 0);								\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	ers_new((A)->sig);														\
...
#elif ALLOC == OPTEE
#define etrs_new(A)															\
	A = (etrs_t)TEE_Malloc(1 * sizeof(etrs_st), 0);									\
	if (A == NULL) {														\
		RLC_THROW(ERR_NO_MEMORY);											\
	}																		\
	bn_new((A)->y);															\
